...
public final class StudentServiceImpl implements IStudentService {
	...
	@Autowired
	@Qualifier("daoAlias")
	//since we have two DAO class(OracleStudentDAOImpl and MysqlStudentDAOImpl) so there is an ambiguity so Qualifier is needed but we not using direct bean ID to resolve this ambiguity
	//instead we are using placeholder to change the dao bean name in Properties file.
